...
# column order. Two doors, shared with eztrieve-dependencies and jcl-dependencies through
...
- **Output is byte-stable and deterministic.** A refactor that should not change output must produce identical bytes — a green test run does not prove this. Verify with **`python tools/gate.py`**, which hashes every view of every `examples/*.cbl` and both retrieval reports, and checks them under two `PYTHONHASHSEED` values, at `--jobs 1` and `8`, and through the parse-bundle round trip (Program → JSON → Program before modelling, against the SAME goldens):
  ...
  Two things genuinely are machine-dependent and are normalized before hashing, rather than pretended away: `copiedTo` in both reports, and the `source` of any member resolved from disk (a copybook row in the artifact manifest carries it, so `<program>::artifacts` embeds a local path). The third thing that *was* machine-dependent no longer is: `.gitattributes` now pins `* text=auto eol=lf`, so every checkout gets LF sources and the `bytes:` count of a copybook read from disk is the same everywhere. It is load-bearing rather than cosmetic — under `core.autocrlf` a Windows checkout made `CUSTREC.cpy` 513 bytes against 503 on Linux, so `cicsinq.cbl`'s two report digests were only ever valid on the checkout they were recorded from. The goldens are recorded from an LF tree, which is what every clone now gets.
