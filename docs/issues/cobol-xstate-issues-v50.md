...
| 3 | Verbs/literals as program endpoints | **Deferred** — per Paul these indicate an upstream misparse to fix at the root; awaiting a better problem description (which source programs produced them). Note: the suspect `_name_suffix` fallback is unreachable from master's CALL provenance spellings, so these may also be stale-build artifacts | Collect the offending source program names from the trace |
